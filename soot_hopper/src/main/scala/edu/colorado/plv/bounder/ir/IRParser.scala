...
object IRParser{
  ...
  def parseReflectiveRef(ref:String):TRef = parser.parseAll(parser.reflectiveRef, ref).getOrElse(
    throw new IllegalArgumentException(s"Failed to parse reflective type reference: $ref")
  )
}
