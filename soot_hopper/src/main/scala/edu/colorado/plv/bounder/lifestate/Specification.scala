...
import edu.colorado.plv.bounder.lifestate.LifeState.{And, Forall, Once, LSConstraint, LSFalse, LSPred, LSSpec, LSTrue, NS, Not, Or, SetSignatureMatcher, SignatureMatcher, SubClassMatcher}
...
object SpecSignatures {
  ...
  val Activity_onResume_entry: Once =
    Once(CBEnter, Activity_onResume, List("_", "a"))
  val Activity_onResume_exit: Once =
    Once(CBExit, Activity_onResume, List("_", "a"))
  val Activity_onCreate_exit: Once =
    Once(CBExit, Activity_onCreate, List("_", "a"))

  val Activity_onCreate_entry: Once =
    Once(CBEnter, Activity_onCreate, List("_", "a"))

  val Activity_onPause: SignatureMatcher =
    SubClassMatcher(Activity,"void onPause\\(\\)", "Activity_onPause")
  val Activity_onPause_entry: Once = Once(CBEnter, Activity_onPause, List("_", "a"))
  val Activity_onPause_exit: Once =
    Once(CBExit, Activity_onPause, List("_", "a"))
...
//    I(CBExit,Activity_init, List("_", "a"))
  ...
  val Activity_onDestroy_exit: Once = Once(CBExit, Activity_onDestroy, List("_","a"))
...
//  val Activity_init_entry: I = Activity_init_exit.copy(mt = CBEnter)
  ...
  val Fragment_get_activity_exit_null: Once = Once(CIExit, Fragment_getActivity, "@null"::"f"::Nil)
  ...
  val Fragment_onActivityCreated_entry: Once = Once(CBEnter, Fragment_onActivityCreated, "_"::"f"::Nil)

  val Fragment_onDestroy: SignatureMatcher = SubClassMatcher(Fragment, "void onDestroy\\(\\)", "Fragment_onDestroy")
  val Fragment_onDestroy_exit: Once = Once(CBExit, Fragment_onDestroy, "_"::"f"::Nil)

  // rxjava
  val RxJava_call: SignatureMatcher = SubClassMatcher("rx.functions.Action1", "void call\\(java.lang.Object\\)", "rxJava_call")

  val RxJava_call_entry: Once = Once(CBEnter, RxJava_call, "_"::"l"::Nil)
  ...
  val RxJava_unsubscribe_exit: Once = Once(CIExit, RxJava_unsubscribe, "_"::"s"::Nil)

  val RxJava_subscribe: SignatureMatcher =
    SubClassMatcher("rx.Single", "rx.Subscription subscribe\\(rx.functions.Action1\\)",
      "RxJava_subscribe")
  val RxJava_subscribe_exit: Once = Once(CIExit, RxJava_subscribe, "s"::"_"::"l"::Nil)
  val RxJava_subscribe_exit_null: Once = RxJava_subscribe_exit.copy(lsVars = "@null"::Nil)
}
...
object LifecycleSpec {
  ...
  val noResumeWhileFinish: LSSpec = LSSpec("a"::Nil, Nil,
    Not(Once(CIExit, SpecSignatures.Activity_finish, "_"::"a"::Nil)),
...
object ViewSpec {
  val anyViewCallin: Once = Once(CIEnter, SubClassMatcher("android.view.View",".*","View_AnyExceptOther"),List("_", "v") )
...
//  val setOnClickListener:LSPred = setOnClickListenerI
  ...
  val clickWhileActive:LSSpec = LSSpec("l"::Nil,"a"::"v"::Nil,
    And(And(setOnClickListener, LifecycleSpec.viewAttached), Or(LifecycleSpec.resumed,
      Once(CIExit, SpecSignatures.Activity_finish, "_"::"a"::Nil))),
    Once(CBEnter, onClick, List("_", "l")))

  val setEnabled:SubClassMatcher = SubClassMatcher(Set("android.view.View"), ".*setEnabled.*", "View_setEnabled")
  private val buttonEnabled = Or(Not(Once(CIExit, setEnabled, "_"::"v"::"@false"::Nil)),
    NS(Once(CIExit, setEnabled, "_"::"v"::"@true"::Nil), Once(CIExit, setEnabled, "_"::"v"::"@false"::Nil))
  )
//  private val buttonEnabled = Not(I(CIExit, setEnabled, "_"::"v"::"@false"::Nil)) //testing version, delete later
  ...
  private val fv_exit = Once(CIExit, SpecSignatures.Activity_findView, "v"::"_"::Nil)
...
object SAsyncTask{
  ...
  private val executeI = Once(CIExit, executeSig, "_"::"t"::Nil)
  private val executeIEntry =
    Once(CIEnter, executeSig, "_"::"t"::Nil)
...
object SDialog{
  ...
  val showI = Once(CIExit, showSignature, "d"::"_"::"a"::Nil)

  val disallowDismiss:LSSpec = LSSpec("d"::Nil, "a"::Nil,
    And(showI, LifecycleSpec.paused),
    Once(CIEnter, dismissSignature, "_"::"d"::Nil))
  val showI2 = Once(CIExit, showSignature, "d"::"_"::Nil)
...
object Dummy{
  val specs = Set(
    ...
    LSSpec("l"::Nil, Nil, LSTrue, Once(CBEnter, SpecSignatures.RxJava_call, "_"::"l"::Nil)), //Dummy call
    LSSpec("v"::"a"::Nil, Nil, LSTrue, SpecSignatures.Activity_findView_exit),
    LSSpec("v"::"l"::Nil,Nil, LSTrue, ViewSpec.setOnClickListenerI),
    LSSpec("l"::Nil, Nil, LSTrue, Once(CBEnter, ViewSpec.onClick, "_"::"l"::Nil))
  )
}