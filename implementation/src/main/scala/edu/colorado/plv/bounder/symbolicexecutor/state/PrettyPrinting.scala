...
import edu.colorado.plv.bounder.ir.{AppLoc, CallbackMethodInvoke, CallbackMethodReturn, CallinMethodInvoke, CallinMethodReturn, CmdWrapper, GroupedCallinMethodInvoke, GroupedCallinMethodReturn, IRWrapper, InternalMethodInvoke, InternalMethodReturn, LineLoc, Loc, SkippedInternalMethodInvoke, SkippedInternalMethodReturn, WitnessExplanation}
...
object PrettyPrinting {
  def getWitness(terminals:Set[IPathNode]):Set[WitnessExplanation] = {
    terminals.flatMap(n => n.qry match {
...
//      .append(traces)
  ...
  def filterObvious(qrySet: Set[IPathNode]):Set[IPathNode] ={
    qrySet.filter{qry =>
      qry.state.sf.pureFormula.forall{
        case PureConstraint(lhs, NotEquals, rhs) if lhs == rhs => false
        case PureConstraint(lhs, op, rhs) => true
      }
    }
  }
  ...
  def dumpDebugInfo(qrySet:Set[IPathNode],
                    ...
                    omitObvious:Boolean = true)(implicit mode : OutputMode = MemoryOutputMode):Unit = {
    val qrySetF = if(!omitObvious) qrySet else filterObvious(qrySet)
    val outDir3 = if(outDir.isDefined) outDir else envOutDir
    outDir3 match{
      case Some(baseDir) =>
