...
\subsection{\req{2} - Viewing results}
...
\newcommand{\numsubset}{\TODO{20}}
The full set of 1090 locations can take an extremely long time to run (over 15 days of CPU time).  Therefore, we recommend running a subset.  The notebook \codej{RunTable2-Minimal-Resources.ipynb} will run this subset by clearing the database and uploading the locations in the \codej{dataSubset} directory.  The worker container will then process them one at a time.  


We chose a set of \numsubset locations that don't use too much ram and don't time out.  Locations are stored in the ``dataSubset'' directory as ``.json'' files.

Progress can be tracked through the \codej{MonitorTable2Run.ipynb} notebook.  All cells in this notebook may be run repeatedly with ``shift-enter'' to track the status and outputs.
...
\TODO{how to actually compare subset to paper?}

\paragraph{Distributing the Verification}
More worker containers may be run in parallel if computing resources are available.  Each worker requires a network connection with port \codej{5432} to the \codej{historia_postgres} container started by \codej{docker-compose.yml}.


\subsection{\req{2} - Table 3}

\subsection{\req{1} - Table 1 - full resources}

\TODO{explain full docker compose running}

In order to run the full version of Table 1, a system with at least 120G of ram must be used. To run, restart the docker container removing the following two arguments from the \codej{docker run} command: \codej{--memory="8G" --memory-swap="8G"} and repeat the instructions from the ``minimal resources'' section.  This should take about an hour to run on a comparable system.
